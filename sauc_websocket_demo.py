...
# 优先从项目根目录的 .env 加载（如果安装了 python-dotenv）
...
class Config:
    def __init__(self):
        # 建议通过环境变量提供，避免把密钥写入代码仓库/日志：
        #   VOLC_ASR_APP_KEY        -> APP ID
        #   VOLC_ASR_ACCESS_KEY     -> Access Token
        #   VOLC_ASR_SECRET_KEY     -> Secret Key（本 demo 不参与鉴权，但保留字段便于你后续扩展）
        #
        # 也可以在命令行显式传入（见 main() 里的参数）。
        self.auth = {
            "app_key": os.getenv("VOLC_ASR_APP_KEY", "").strip(),
            "access_key": os.getenv("VOLC_ASR_ACCESS_KEY", "").strip(),
            "secret_key": os.getenv("VOLC_ASR_SECRET_KEY", "").strip(),
        }
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise RuntimeError(
                "Missing auth config. Please set environment variables: "
                + ", ".join(missing)
            )
...
class RequestBuilder:
    @staticmethod
    def new_auth_headers(resource_id: str) -> Dict[str, str]:
        # 文档要求：X-Api-App-Key / X-Api-Access-Key / X-Api-Resource-Id / X-Api-Connect-Id
        connect_id = str(uuid.uuid4())
        request_id = str(uuid.uuid4())
        return {
            "X-Api-Resource-Id": resource_id,
            "X-Api-Connect-Id": connect_id,
            # 有些排障/链路场景会用到 request id；服务端若不识别会忽略
...
class AsrWsClient:
    def __init__(self, url: str, segment_duration: int = 200, resource_id: str = "volc.bigasr.sauc.duration"):
        self.seq = 1
        self.url = url
    ...
    async def create_connection(self) -> None:
        ...
        try:
            ...
            # 握手成功后，服务端会在 Response Header 里返回 logid / connect id（用于排障）
            try:
                resp_headers = getattr(self.conn, "headers", {}) or {}
                ...
                if connect_id:
                    logger.info(f"Handshake X-Api-Connect-Id: {connect_id}")
                if logid:
                    logger.info(f"Handshake X-Tt-Logid: {logid}")
            except Exception as e:
                logger.warning(f"Failed to read handshake headers: {e}")
        except Exception as e:
...
async def main():
    ...
    env_url = os.getenv("VOLC_ASR_WS_URL", "").strip()
    env_resource_id = os.getenv("VOLC_ASR_RESOURCE_ID", "").strip()
    env_seg_duration = os.getenv("VOLC_ASR_SEG_DURATION_MS", "").strip()
    try:
        env_seg_duration_int = int(env_seg_duration) if env_seg_duration else None
    except ValueError:
        env_seg_duration_int = None
    ...
    parser.add_argument("--url", type=str, default=(env_url or "wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_nostream"),
                       help="WebSocket URL")
    parser.add_argument(
        "--resource-id",
        type=str,
        default=(env_resource_id or "volc.bigasr.sauc.duration"),
        help="X-Api-Resource-Id (hour-based: volc.bigasr.sauc.duration / volc.seedasr.sauc.duration)",
    )
    parser.add_argument("--seg-duration", type=int, default=(env_seg_duration_int or 200),
                       help="Audio duration(ms) per packet, default:200")
    parser.add_argument("--app-key", type=str, default="", help="Override VOLC_ASR_APP_KEY")
    parser.add_argument("--access-key", type=str, default="", help="Override VOLC_ASR_ACCESS_KEY")
    
    args = parser.parse_args()

    if args.app_key.strip():
        config.auth["app_key"] = args.app_key.strip()
    if args.access_key.strip():
        config.auth["access_key"] = args.access_key.strip()
    config.validate()
    
    async with AsrWsClient(args.url, args.seg_duration, resource_id=args.resource_id) as client:  # 使用async with
        try:
